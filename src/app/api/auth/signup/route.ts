...
import { signupLimiter, clientIpOrFail, clientIp } from '@/lib/ratelimit'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logError('/api/auth/signup', e, { context: 'ratelimit error' })
    return NextResponse.json(
  ...
  const { username, email, password, country = 'XX', turnstileToken } = body as Record<string, unknown>

  // Bot challenge — closes audit AU-02. Cloudflare Turnstile token must
  // verify before we touch bcrypt (expensive) or the DB.
  if (!(await verifyTurnstile(turnstileToken as string | undefined, clientIp(req)))) {
    return NextResponse.json({ error: 'Bot challenge failed — refresh and try again' }, { status: 403 })
  }
  ...
  try {
    ...
    logError('/api/auth/signup', err, { context: 'insert failed' })
    throw err
  }
