...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    logError('/api/profile/settings', e, { context: 'settingsLimiter error' })
  }
  ...
  if (field === 'username') {
    ...
    const [existing] = await db
      .select({ id: users.id })
      .from(users)
      .where(and(eq(users.username, value), isNull(users.deletedAt)))
      .limit(1)
    ...
    try {
      ...
      logError('settings', err, { context: 'leaderboard rewrite failed' })
    }

    // 3) Bump the rename tag so the JWT callback picks up the new username on
    //    the user's next request — replacing the old 5s DB polling loop.
    try {
      await redis.set(`u:renametag:${session.user.id}`, crypto.randomUUID(), { ex: 86400 })
    } catch (err) {
      logError('settings', err, { context: 'rename tag set failed' })
    }
  ...
  if (field === 'password') {
    try {
      ...
      logError('/api/profile/settings', e, { context: 'passwordChangeLimiter error' })
      return NextResponse.json(
    ...
    const pwCheck = await checkPassword(value)
    if (!pwCheck.ok) {
      return NextResponse.json({ error: pwCheck.error ?? 'Invalid password' }, { status: 400 })
    }
...
export async function DELETE(req: NextRequest) {
  ...
  // Soft-delete: mark deletedAt, scrub auth fields, keep the row.
  //
  // Why soft-delete: hard-deleting frees the username immediately, which
  // would let another user register the same name and inherit the public
  // leaderboard entries that still link to it (audit finding C-13).
  // The row stays so the username remains reserved. A future cron can
  // hard-delete rows older than 30 days to free the name for re-use.
  //
  // Why scramble email + auth fields: removes everything that could let a
  // ghost session re-authenticate, while keeping the row.
  await db
    .update(users)
    .set({
      deletedAt: new Date(),
  ...
  // Invalidate the JWT cookie so the session ends immediately and a ghost
  // token cannot replay against authenticated endpoints (audit C-11).
