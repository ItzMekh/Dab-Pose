...
// Lazy-load: only fetched when a Google first-time sign-in needs to
// choose a username — most landing visits never load this chunk.
const UsernameSetupModal = dynamic(() => import('./UsernameSetupModal'), {
  ssr: false,
})
  ...
  return (
    <div className="relative text-center space-y-10 p-4 sm:p-8">
        ...
        {session?.user?.name && !session.user.needsUsernameSetup ? (
          ...
          <Link href="/login" className="group block">
            <div className="flex flex-col items-center gap-1.5 bg-black/80 backdrop-blur-sm border border-white/10 group-hover:border-purple-500/40 group-hover:shadow-[0_0_16px_rgba(168,85,247,0.15)] rounded-2xl px-2.5 py-2.5 sm:px-3 sm:py-3 w-[76px] sm:w-[88px] transition-all duration-200">
              ...
              <p className="text-gray-400 text-[10px] leading-tight text-center">Track dabs</p>
            </div>
          </Link>
        )}
      </motion.div>
      ...
      >
        ...
        <span className="text-gray-400 text-sm font-normal">Top Dabbers →</span>
      </motion.a>
      ...
      <motion.footer
        ...
        className="text-xs text-gray-400 flex justify-center gap-4 pb-2"
      >
